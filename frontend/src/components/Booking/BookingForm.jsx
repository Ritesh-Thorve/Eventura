  ...
  export function BookingForm({ venue, onClose }) {
    ...
    // Automatically set logged-in user's email, name, and userId when component mounts
    useEffect(() => {
      if (user) {
        setFormData((prev) => ({
          ...prev,
    ...
    const handleSubmit = async (e) => {
      ...
      if (!formData.userId || !formData.venueName || !formData.email || !formData.eventType || !formData.eventDate || !formData.phone || !formData.name || !formData.location) {
      ...
      try {
        ...
        if (response.status === 201) {
          toast.success('Application for appointment submitted successfully! We will contact you shortly.', {
            position: 'top-right',
            autoClose: 5000,
